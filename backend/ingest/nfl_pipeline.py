...
def run_nfl_pipeline() -> dict:
    logger.info("nfl_pipeline_start")
    ...
    for team in standings:
        ...
        pt_diff = team.get("point_diff", 0)

        elo_rating = 1500 + (win_pct - 0.5) * 400 + pt_diff * 0.3
        elo_score = max(0, min(100, (elo_rating - 1350) / 350 * 100))

        eid = team.get("espn_id", "")
        inj_list = injuries_by_team.get(eid, [])

        teams_input.append({
            "team_id": eid or team.get("abbreviation", ""),
            "abbreviation": team.get("abbreviation"),
            "full_name": team.get("display_name"),
            "gg_elo": round(elo_rating, 2),
            "gg_elo_score": round(elo_score, 2),
            "nir_score": round(50 + win_pct * 50, 2),
            "ssc_score": 50.0,
            "iis_score": max(0, 100 - len(inj_list) * 6),
    ...
    for team in ranked:
        ...
        if not book_odds:
            continue
        champ_prob = team["cfs_score"] / 100 * 0.20
        ...
        if mid["mid_edge"] >= NEEDLE_EDGE_THRESHOLD:
            ...
            try:
                ...
                logger.warning("needle_insert_failed", sport="nfl", error=str(e))

    try:
        save_snapshot("nfl", SEASON, "weekly", {
            "teams": ranked[:10],
            "champion_probs": {t["abbreviation"]: round(t["cfs_score"] / 100 * 0.20, 4) for t in ranked},
        }, ranked[0]["abbreviation"] if ranked else None)
    except Exception as e:
        logger.warning("snapshot_save_failed", error=str(e))

    logger.info("nfl_pipeline_complete", teams=len(ranked), needles=needle_count)
